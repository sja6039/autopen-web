...
app.post("/api/autopen/upload", upload.single("file"), async (req, res) => {
  ...
  if (!piBase) {
    return res
      .status(500)
      .json({ error: "AUTOPEN_HMI_URL is not configured on the server." });
  }
  ...
  if (!code || !/^\d{6}$/.test(code)) {
    return res
      .status(400)
      .json({ error: "A valid 6-digit pairing code is required." });
  }
  ...
  try {
    ...
    if (err.name === "AbortError") {
      return res.status(504).json({
        error:
          "Request to the Autopen Pi timed out. Is it on the same network?",
      });
    }
    return res.status(502).json({
      error: "Could not reach the Autopen Pi. Check AUTOPEN_HMI_URL.",
    });
  }
});
...
app.listen(PORT, "0.0.0.0", () => {
  ...
  if (!process.env.AUTOPEN_HMI_URL) {
    console.warn(
      "  ⚠  AUTOPEN_HMI_URL is not set — uploads will fail until it is.",
    );
  } else {
